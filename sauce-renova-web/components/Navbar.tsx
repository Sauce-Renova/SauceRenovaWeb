...
export default function Navbar() {
  ...
  return (
    <nav className="bg-green w-full px-6 py-3 flex items-center rounded-full">
      ...
      <ul className="flex items-center gap-8 ml-auto">
        {navLinks.map((link) => (
          ...
          >
            <Link
              href={link.href}
              className="text-white font-bold text-sm tracking-wide hover:opacity-80 transition-opacity"
            >
                ...
                {link.dropdown.map((item) => (
                  <li key={item}>
                    <Link
                      ...
                      className="block px-5 py-2 text-white font-bold text-sm hover:opacity-80 transition-opacity"
                    >
                      {item.toUpperCase()}
                    </Link>
                  </li>
